...
def add_detail(vault: Dict, username: str, vault_key: bytes) -> None:
    """
    ...
    Prompts for:
      ...
      • Detail type
      • Detail value
    Encrypts and stores under vault['details_vault'][username][service][detail_type].
    """
    ...
    # Audit log: record a successful detail addition
    log_event(vault, username, "add_detail", True, {"service": service, "detail_type": detail_type})


def get_details(vault: Dict, username: str, vault_key: bytes) -> None:
    """
...
def del_detail(vault: Dict, username: str) -> None:
    """
    ...
    if detail_type in user_store[service]:
        ...
        # Log the detail deletion.
        log_event(vault, username, "delete_detail", True, {"service": service, "detail_type": detail_type})
    else:
        print("❌ Detail type not found.\n")


def edit_detail(vault: Dict, username: str, vault_key: bytes) -> None:
    """
    ...
    if new_val:
        ...
        # Log the detail edit event.
        log_event(vault, username, "edit_detail", True, {"service": service, "detail_type": detail_type})
    else:
        print("No changes made.\n")


def details_menu(vault: Dict, username: str, vault_key: bytes) -> None:
    """
    Looping menu for details-vault operations.
    Starts with a clear screen on each iteration.
    """
    while True:
        clear_screen()  # Ensures a fresh view for the details menu.
        ...
        if choice == "1":
            add_detail(vault, username, vault_key)
            input("Press Enter to continue...")
        elif choice == "2":
            get_details(vault, username, vault_key)
            input("Press Enter to continue...")
        elif choice == "3":
            del_detail(vault, username)
            input("Press Enter to continue...")
        elif choice == "4":
            edit_detail(vault, username, vault_key)
            input("Press Enter to continue...")
        elif choice == "5":
            break
        else:
